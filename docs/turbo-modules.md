...
## Add Event Emitting Capabilities
...
This capability can be helpful to notify the JS layer about changes in the platforms (for example a change in appearence) or for tasks that ended asynchronously.
It is also a great way to send events from your modules to your components.

To add this feature, we have to apply some changes at the various layers.

### 1. Add the Event Emitter to the JS Spec.

First of all, our specs needs to define an EventEmitter function.

Modify the specs as it follows:

<details>
  <summary>Flow</summary>

```diff title="NativeCalculator.js with Event Emitter"
// @flow
import type {TurboModule} from 'react-native/Libraries/TurboModule/RCTExport';
import {TurboModuleRegistry} from 'react-native';
...
```diff title="NativeCalculator.ts with Event Emitter"
import { TurboModule, TurboModuleRegistry } from "react-native";
+ import type {EventEmitter} from 'react-native/Libraries/Types/CodegenTypes';
...
This new property will tell codegen to generate all the new properties and methods to wire the Event Emitter in native code

### 2. Run Codegen

This step is the same step of the previous guide.
...
In this case, the Codegen will create a new base class called `NativeRTNCalculatorSpecBase` that contains the glue code that allows the Event Emitter to work properly.
...
For Android, the output is not much different. The classes that were generated before will now contains the new code to make the Event Emitter work.

### 3. Update the Native Code

Now that Codegen generated some new code, we need to update our existing native code to use the newly generated code.

#### iOS

For iOS, we need two steps:
1. inherit from the newly created class
2. emit the event

##### Inherith from the Newly created class

1. Open the `RTNCalculator.h` file
2. Modify it as it follows:

```diff title="RTNCalculator.h with Event Emitter"
#import <RTNCalculatorSpec/RTNCalculatorSpec.h>
...
1. Open the `RTNCalculator.mm` file
2. Modify it as it follows:

```diff title="RTNCalculator.mm with Event Emitter"
#import "RTNCalculator.h"
...
On Android, the CalculatorModule already inherits from the right class. So, the only step required is to emit the event.

1. Open the `CalculatorModule.java` file
2. Modify it as it follows:

```diff title="CalculatorModule.java"
...
### 4. Listen for the Event in JS.

Now that all the Native Code is properly wired, we only need to listen for the event on JS side.

To do so, we can modify our app code as it follows:
...
+  React.useEffect(() => {
+    listenerSubscription.current = RTNCalculator.onValueChanged((data) => { Alert.alert(`Result: ${data}`) });
...
1. We imported the `EventSubscription` type from React Native to handle the subscription to the Event properly.
2. Then we defined a `listenerSubscription` object using `useRef` to keep track of the subscription.
3. We used a `useEffect` hook to register to the event emitter as soon as the component is created. Notice that we are returning a cleanup closure to remove the listener and to cleanup the ref, so we don't have memory leaks in our app.
4. We added the listener to the module by using the `RTNCalculator.onValueChanged` method we defined in the specs. This method returns an `EventSubscription` that keeps track of the listener.

Now, every time the user computes something, an Alert will pop up with the result.
